...
@Mapper
public interface GRPCBlobStorageClientMapper {
    ...
    default BlobStorageApiV1.PutObjectReq map(StorageKey storageKey, StorageData storageData) {
        var storageKeyReq = map(storageKey);
        var storageDataReq = map(storageData);
        return BlobStorageApiV1.PutObjectReq.newBuilder()
                .setStorageKey(storageKeyReq)
                .setStorageData(storageDataReq)
                .build();
    }
    ...
    default BlobStorageApiV1.StorageKey map(StorageKey storageKey) {
        return BlobStorageApiV1.StorageKey.newBuilder()
                .setExchange(storageKey.exchange())
                .setKey(storageKey.key())
                .build();
    }
